...
function getVisualizedMesh(
  ...
  for (i in bin) {
    ...
    var relevantExportCategory =
      relevantExport && $.inArray(categoryName, exportCategories) >= 0;
    var relevantImportCategory =
      relevantImport && $.inArray(categoryName, importCategories) >= 0;

    if (
      (useImporter || useExporter) &&
      (relevantExportCategory || relevantImportCategory)
    ) {
      ...
      var lineColor = thisLineIsExport
        ? new THREE.Color(exportColor)
        : new THREE.Color(importColor);
      ...
      var particleCount =
        Math.floor(set.v / 8000 / set.lineGeometry.vertices.length) + 1;
      ...
      if (
        importerCountry == selectedCountry ||
        exporterCountry == selectedCountry
      ) {
