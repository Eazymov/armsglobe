...
window.onload = function start () {
  ...
  if (!Detector.webgl) {
    ...
    mapIndexedImage.src = "../assets/img/map_indexed.png";
    mapIndexedImage.onload = function() {
      mapOutlineImage = new Image();
      mapOutlineImage.src = "../assets/img/map_outline.png";
...
function animate() {
  if (rotateTargetX !== undefined && rotateTargetY !== undefined) {
    ...
    if (
      Math.abs(rotateTargetX - rotateX) < 0.1 &&
  ...
  THREE.SceneUtils.traverseHierarchy(rotating, function(mesh) {
...
function highlightCountry(countries) {
  ...
  for (var i in countryCodes) {
    ...
    // var fillCSS = '#ff0000';
    var fillCSS = "#333333";
    if (countryCode === selectedCountryCode) fillCSS = "#eeeeee";
  ...
  for (var i in timeBins) {
    ...
    for (var s in yearBin) {
      ...
      var relevantCategory =
        (countryName == exporterCountryName &&
          $.inArray(categoryName, exportCategories) >= 0) ||
        (countryName == importerCountryName &&
          $.inArray(categoryName, importCategories) >= 0);

      if (relevantCategory == false) continue;

      //	ignore all unidentified country data
      if (
        countryData[exporterCountryName] === undefined ||
        countryData[importerCountryName] === undefined
      )
        continue;

      if (exporterCountryName == countryName) value.exports += set.v;
      if (importerCountryName == countryName) value.imports += set.v;
    }
    history.push(value);
  }
  // console.log(history);
